...
def parse_questions_file(filepath, max_questions=200):
...
def load_all_categories():
    """Загружает все категории без перевода"""
    ...
    # Список всех категорий с приоритетом по размеру
    categories_priority = [
        ('general', 300),           # Самые популярные
    ...
    for category, max_questions in categories_priority:
...
def generate_js_file(data):
    ...
    js_content = "// Полная база данных квизов из OpenTriviaQA (английский язык)\n"
...
def main():
    print("📚 Загружаем ВСЕ вопросы из OpenTriviaQA датасета (английский язык)...")
    ...
    # Сохраняем в файл
    with open('quiz-data-full-english.js', 'w', encoding='utf-8') as f:
        f.write(js_content)
    
    print(f"\n💾 Полная база данных сохранена в quiz-data-full-english.js")

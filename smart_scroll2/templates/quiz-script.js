...
class QuizApp {
    ...
    navigateToPage(page) {
        ...
        switch(page) {
            case 'feed':
                window.location.href = `${baseUrl}:${port}/web/feed.html`;
                break;
            case 'posts':
                window.location.href = `${baseUrl}:${port}/web/posts.html`;
                break;
            case 'quiz':
                // Остаемся на текущей странице
                break;
            case 'settings':
                window.location.href = `${baseUrl}:${port}/web/settings.html`;
                break;
            case 'profile':
                // Пока нет страницы профиля, перенаправляем на settings
                window.location.href = `${baseUrl}:${port}/web/settings.html`;
                break;
        }
    }
